-- Vendored from https://github.com/shezdy/hyprsplit @ 6b00b67 (init.lua only —
-- the rest of that repo builds the deprecated C++ plugin, which we do not use).
--
-- Local patch: log() writes to stderr instead of stdout, so hyprsplit's output
-- does not land in Hyprland's own stdout stream. Re-apply it after updating:
--   curl -L https://raw.githubusercontent.com/shezdy/hyprsplit/main/init.lua -o init.lua
...
local function log(formatString, ...)
    io.stderr:write(string.format("[hyprsplit] " .. formatString, ...) .. "\n")
end
